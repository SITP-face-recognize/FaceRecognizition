...
int main()
{
	...
	while (1) {
		cout << "功能选择" << endl;
		cout << "录入----0" << endl << "训练----1" << endl << "识别----2" << endl << "退出----3"<< endl;
		cin >> inputKey;
		switch (inputKey) {
		case '0':shoot(); preprocessing(); csv(); break;
		case '1':mmodel_training(); break;
		case '2': faceRecognition(); break;
		case '3':return 0;
		default:break;
		}
...
string getTag(string s) {;             //给定路径获取文件名前六位（学号）
	int pos;
	pos=s.find_last_of('\\');
...
void detectROIandResize(string imagePath, CascadeClassifier cascade, int i)
{
	...
	cvtColor(image_gray, image_gray, COLOR_BGR2GRAY); //转灰度图
...
int preprocessing() {
	...
	folderPath = "E:\\pic";
	string cascadeName;
	cascadeName = "D:\\opencv\\sources\\data\\haarcascades\\haarcascade_frontalface_alt.xml";
	vector<string> files;
...
int faceRecognition()
{
	...
	while (1)
	{
		...
		if (predictPCA != 0)
		cout << predictPCA <<"   置信度:"<<confidence<< endl;	
